<?php
/*
Plugin Name: Sidebar Form
Plugin URI: http://www.kenmoredesign.com/
Description: Standard Sidebar form on any page or post of your website. Simply insert [SIDEBAR-FORM] within any post or page.
...
function ContactUsForm()
{
	...
	add_shortcode('SIDEBAR-FORM', array( &$this, 'shortcode'));

	add_action('wp_head', array( &$this, 'addStyle'));

	if ( function_exists('register_uninstall_hook') )
		register_uninstall_hook(ABSPATH.PLUGINDIR.'/sidebar-form/sidebar-form.php', array( &$this, 'uninstall')); 
...
function showFormWidget( $params = '' )
{
	...
	if ( empty($result) || (!empty($result) && !$this->o['hideform']) )
	{
		...
		$form .= '
			<form style="width:200px;" action="#cuform'.$n.'" method="post" id="tinyform'.$n.'">
			<div>
			<input name="cuf_name'.$n.'" id="cuf_name'.$n.'" value="" class="cuf_input" />
			<input name="cuf_sendit'.$n.'" id="cuf_sendit'.$n.'" value="1" class="cuf_input" />
			<label for="cuf_sender'.$n.'" class="cuf_label">'.__('Name', 'cuf-lang').':</label>
...
function sendMail( $n = '', $params = '' )
{
    ...
    if ( $result == 'OK' )
    {
		...
		if ( mail( $to, $subject, $fullmsg, $headers, $email) )
		{
			if ( $this->o['hideform'] )
			{
...
{	
	...
	<div id="poststuff" class="wrap">
    	...
    	<tr>
	    <th>   </th>
	    	<td>To insert the form on the page simply pate the following shortcode in the HTML: <b>[SIDEBAR-FORM]</b></td>
		<tr>
    	...
    	<tr>
			...
			<td>
				<p><?php _e('The sidebar form includes the fields Name, Email, Subject and Message. To add more fields simply add them below', 'cuf-lang'); ?></p>
				<?php
				for ( $x = 1; $x <= 5; $x++ )
...
class ContactUsForm_Widget extends WP_Widget
{
	...
	function ContactUsForm_Widget() {
		parent::WP_Widget('cuform_widget', 'Sidebar Form', array('description' => 'Sidebar Form'));	
	}
	...
	function widget( $args, $instance)
	{
		...
		echo $contact_us_form->showFormWidget( $instance );
		echo $after_widget;
	}
	...
	{
		$default = array('title' => 'Sidebar Form');
